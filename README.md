...
Install root resolution (current implementation):
- If current working directory contains `skills/` or `agents/`, that directory is used as install root.
- Otherwise installer falls back to the CLI package root (the directory that contains the built installer), and writes `.claude`, `.gemini`, `.codex` there.

This means running from an unrelated directory does **not** guarantee install to that directory.
